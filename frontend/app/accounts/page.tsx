...
export default function AccountsPage() {
  ...
  async function sync(id: string) { if (syncing.current.has(id)) return; syncing.current.add(id); setBusy(id); setError(null); try { await syncAccount(id); await load(); } catch { setError("Sync failed. Try again in a moment."); } finally { syncing.current.delete(id); setBusy(null); } }
